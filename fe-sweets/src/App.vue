...
export default {
	...
	methods: {
		updateShop() {
			...
			this.count ++;
			this.subTotal();
			axios.get(`https//localhost:8080/stock`).then(res => {
				console.log(res)
		...
		sell(item) {
			axios.delete(`https//localhost:8080/${item.id}`).then(
				this.updateShop()
			);
		},
		order(item) {
			axios.post(`https//localhost:8080/${item.id}/restock`).then(
				this.updateShop()
			);
		},
		subTotal() {
			axios.get(`https//localhost:8080/value`).then(res => {
				this.total = res.data;
			});
		}
	},
