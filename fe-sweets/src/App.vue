...
export default {
	...
	methods: {
		...
		sell(item) {
			axios.delete(`https://perminder.developyn.com/${item.id}`).then(
				this.updateShop()
			);
		},
		order(item) {
			axios.post(`https://perminder.developyn.com/${item.id}/restock`).then(
				this.updateShop()
			);
		},
		subTotal() {
			axios.get(`https://perminder.developyn.com/value`).then(res => {
				this.total = res.data;
			});
		}
	},
